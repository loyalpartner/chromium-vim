...
def compute_ninja_command_for_current_buffer():
  ...
  build_cmd = ' '.join(['autoninja', '-v', '-C', build_dir, file_to_build + '^'])
...
fun! CrCompileFile(dry_run)
  if a:dry_run
    let @+=s:NinjaCommandForCurrentBuffer()
  else
...
command! CrCompileFile call CrCompileFile(v:false)
command! CrBuildCommandLine call CrCompileFile(v:true)
